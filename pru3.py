...
def guardar_campus():#Guardar los elementos de la lista campus en un archivo de texto.
...
def cargar_campus():
    if os.path.exists("campus.txt"):
        with open("campus.txt", "r") as f:
            return [line.strip() for line in f.readlines()]# Lee todas las líneas del archivo, elimina los saltos de línea y retorna la lista de líneas.
    else:
        return []

campus = cargar_campus()

def validar_ip_multiface(ip_str):
    """Valida una dirección IP con el formato INTERFACE: IP/MASK"""
    try:
        interface, ip_mask = ip_str.split(":")
        ipaddress.ip_interface(ip_mask.strip())
        return True
    except ValueError:
        return False
...
def menu_ver_disp():
    while True:
        os.system("clear")
        print("Seleccione el campus para ver dispositivos.")
        ...
        try:
            ...
            if selector == len(campus) + 1:
                ...
                with open(file_path, "a") as file:
                    ...
                    print("a. Tipo de dispositivo:")
                    for key, value in tipo_opciones.items():
                        print(f"{key}. {value}")
                    while True:
                        try:
                            tipo_dispositivo = input("Elija una opcion (o 'q' para salir): ").lower()
                            if tipo_dispositivo == 'q':
                                return
                            tipo_dispositivo = int(tipo_dispositivo)
                            if tipo_dispositivo in tipo_opciones:
                                tipo_dispositivo_str = tipo_opciones[tipo_dispositivo]
                                break
                            else:
                    ...
                    file.write(f"\n---------------------------------\n")
                    file.write(f"Tipo de dispositivo: {tipo_dispositivo_str}\n")
